...
module.exports = {
    ...
    ValidateCart: async (req, res) => {
        ...
        try {
            ...
            let calculateProductTotals = (variant, qty, activeServices = []) => {
                ...
                return { total: total.toFixed(2), discount: discount.toFixed(2), final: final.toFixed(2) };
            };
            let recalcCartTotals = (cart) => {
                let total = 0,
                    discount = 0,
                ...
                cart.DiscountCartPrice = discount.toFixed(2);
                cart.FinalCartPrice = final.toFixed(2);
            };
        ...
        try {
            ...
            const calculateTotals = (variant, qty, addOnServices = []) => {
                ...
                if (Array.isArray(addOnServices) && addOnServices.length) {
                    const serviceTotal = addOnServices.reduce((sum, s) =>
                        sum + Number(s.ProductServiceAmount ?? s.serviceData?.ProductServiceAmount ?? 0), 0);
            ...
            if (VariantData.InventoryBaseStock?.InventoryBase === true) {
                await VariantProduct.updateOne(
                    { _id: VariantData._id, "InventoryBaseStock.InventoryBase": true },
                    {
                        $inc: {
                            "InventoryBaseStock.AvailableStock": -Quantity,
                            "InventoryBaseStock.ReservedStock": Quantity
    ...
    proceedToPaymentForCart: async (req, res) => {
        ...
        try {
            ...
            try {
                ...
                let FoundedAddress;
                if (!AddressId) {
                    FoundedAddress = FoundUser.Address.find(a => a.DefaultAddress === true) || FoundUser.Address[0];
                } else {
                    FoundedAddress = FoundUser.Address.find(a => a._id == AddressId);
                }
                if (!FoundedAddress)
                    return res.status(400).json({ message: 'address not found', success: false });
                ...
                if (data && data[0]) {
                    ...
                    for (let p of data[0].Products) {
                        ...
                        let FoundBrand = await brandmodel.findOne({ companyId, _id: p.ProductInfo.BrandId });
                        if (FoundBrand) {
            ...
            for (let Product of FoundOrder.Products) {
                let FoundedProduct = FoundCart.Products.find(
                    (EachProduct) => Product.CartProductId.toString() === EachProduct._id.toString()
                );
                if (FoundedProduct) {
    ...
    getOrders: async (req, res) => {
        try {
            ...
            const allowedPaymentGateways = ['Paytm', 'COD'];

            if (PaymentGateway && !allowedPaymentGateways.includes(PaymentGateway)) {
                return res.status(400).json({ message: `Invalid PaymentGateway. Allowed: ${allowedPaymentGateways.join(', ')}`, success: false });
            }
            ...
            let matchCondition = {
                ...
                $or: [
                    ...
                    { "PaymentSession.paymentGateway": "Paytm", "PaymentSession.status": "SUCCESS" }
                ]
            };
            ...
            if (PaymentGateway === "COD") {
                ...
                matchCondition["PaymentSession.paymentGateway"] = "COD";
            } else if (PaymentGateway === "Paytm") {
                delete matchCondition.$or;
                matchCondition["PaymentSession.paymentGateway"] = "Paytm";
                matchCondition["PaymentSession.status"] = "SUCCESS";
            }

            if (PaymentType) {
                matchCondition["PaymentSession.paymentType"] = PaymentType;
            }
            ...
            let FilteredOrders = Orders.map(order => {
                ...
                if (ProductOrderId) {
                    if (!mongoose.isValidObjectId(ProductOrderId)) return { ...order.toObject(), Products: [] };
            ...
            if (!FilteredOrders.length)
                return res.status(404).json({ message: "No orders matched the given filters", success: false });
            ...
            console.error("getOrdersError:", error.message);
    ...
    getAllOrders: async (req, res) => {
        try {
            ...
            const allowedPaymentStatuses = ['INITIATED', 'SUCCESS', 'FAILED', 'PENDING', 'EXPIRED'];
            const allowedPaymentGateways = ['Paytm', 'COD'];

            if (PaymentStatus) {
                if (!allowedPaymentStatuses.includes(PaymentStatus)) {
                    return res.status(400).json({
                        message: `Invalid PaymentStatus. Allowed: ${allowedPaymentStatuses.join(', ')}`,
                        success: false
                    });
                }
            ...
            if (PaymentGateway) {
                if (!allowedPaymentGateways.includes(PaymentGateway)) {
                    return res.status(400).json({
                        message: `Invalid PaymentGateway. Allowed: ${allowedPaymentGateways.join(', ')}`,
                        success: false
                    });
                }
                matchCondition["PaymentSession.paymentGateway"] = PaymentGateway;
            }
            ...
            if (!FilteredOrders.length) {
                return res.status(404).json({
                    message: "No orders matched the given filters",
                    success: false
                });
            }
            ...
            console.error("getAllOrdersError:", error.message);
            return res.status(500).json({
                message: "Internal Server Error",
                error: error.message,
                success: false
