            ...
            let recalcTotals = (wl) => {
                ...
                for (let p of wl.Products) {
                    if (p.IsActive !== false) {
                        ...
                        discount += Number(p.DiscountPrice || 0);
                        final += Number(p.FinalPrice || 0);
                    }
                }

                wl.TotalCartPrice = total.toFixed(2);
                wl.DiscountCartPrice = discount.toFixed(2);
                wl.FinalCartPrice = final.toFixed(2);
            };
